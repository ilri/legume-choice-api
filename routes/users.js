...
router.route("/deletemany").post(async (req, res) => {
	...
	const deleteEmails = deleteUsers.map((user)=> {return user.email})
	const deleteUsernames = deleteUsers.map((user)=>{return user.username})
	result = await Users.deleteMany({username: {$in: deleteUsernames},email: {$in: deleteEmails}})
	res.send(result)
	}catch(err){
	res.send(err)
	}
